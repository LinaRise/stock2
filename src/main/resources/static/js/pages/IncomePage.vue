...
<script>
    export default {
        ...
        methods: {
            idSelected() {
                this.billProducts = [];
                this.$resource("/incomeBill{/id}").get({id: this.selectedId}).then(result =>
                    result.json().then(data => {
                        this.selectedBill = data;
                        data.incomeStrings.forEach(element => {
            ...
            productSave() {
                const incomeString = {
                    ...
                    income_bill: this.selectedBill
                };
                if (this.editedProductIndex > -1) {
                    Object.assign(this.billProducts[this.editedProductIndex], this.editedProduct);
                    this.$resource("/incomeStrings{/id}").update({id: this.editedProduct.id}, incomeString);
                } else {
                    this.$resource("/incomeStrings").save(incomeString).then(result =>
            ...
            deleteProduct(item) {
                ...
                if (isDeleted) {
                    this.$resource("/incomeStrings{/id}").remove({id: item.id});
                }
            },
            billClose() {
            ...
            billSave() {
                this.$resource("/incomeBill").save(this.editedBill).then(result =>
            ...
            delSelected() {
                this.$resource("/incomeBill{/id}").get({id: this.deletionId}).then(result =>
                    result.json().then(data => {
            ...
            deleteSave() {
                this.$resource("/incomeBill{/id}").remove({id: this.deletionId}).then(result => {
                    if (result.ok) {
        ...
        created() {
            this.$resource("/incomeBill").get().then(result =>
                result.json().then(data => {
